...
\section{DAG multiple testing procedure}
...
To get the implications from a \Robject{DAG} object, we can use the \Rfunction{implications}. This function only requires a \Robject{DAG} object as argument. In our example, the implications look like this: 
